...
class DataTransformation:
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            target_column_name = ["Unnamed: 0","Good/Bad"]
            target_column_encode = "Good/Bad"
            target_column_mapping = {1: 0, -1: 1}

            #training dataframe
            input_feature_train_df = train_df.drop(columns=target_column_name, axis=1)
            target_feature_train_df = train_df[target_column_encode].map(target_column_mapping)

            #testing dataframe
            input_feature_test_df = test_df.drop(columns=target_column_name, axis=1)
            target_feature_test_df = test_df[target_column_encode].map(target_column_mapping)
